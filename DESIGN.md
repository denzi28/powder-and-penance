...
- **Inventory and equip load.** You keep everything you pick up: weapons, shields and armour go in your pack (`player.inv`). Two hand slots, a shield, and head and body armour are filled from it on the EQUIPMENT screen (Esc, pause menu). Weapons picked up go straight to an empty hand; with both hands full they go in the pack. G still drops the weapon in hand.
  - Every weapon, shield and armour piece has a `weight`. What you have *equipped* adds up to your equip load against `capacity` (`data/config/load.json`, 30). The fraction sets a tier: **light** (≤30%) is a quick, long roll with 2 more i-frames and 10% cheaper stamina; **medium** (≤70%) is the normal roll; **heavy** (≤100%) is a short, slow roll, 3 fewer i-frames, 25% dearer, and walking at 92%; **overloaded** means a clumsy stumble instead of a roll, no sprinting, and walking at 60%. `rollFor(tier)` reshapes `roll.json` (distance, travel and recovery time, i-frames, cancel windows) so the tiers stay consistent.
  - Armour (`data/armour/*.json`, `slot` head or body) takes a fraction of damage off (`absorb`, pieces add up, 50% at most) and adds `poise`. Each piece repaints the player's cloak with its colour `ramp` (the hood for head armour): black robes, leather, grey mail.
  - Gear is found in chests (`gear` items): the Iron Buckler on the road, the Pilgrim's Hood under the gibbets, the Wick Knife and Drowned Veil in the Mire, the Gaoler's Helm, the Renderer's Apron and the Parish Revolver in the Works, the Warden's Hauberk, the Bellfounder's Axe, the Acolyte's Robe and the Heavy Crossbow in the Abbey.
  - The INVENTORY screen lists weapons, shields, armour, items (phial, tallow, rounds) and keys by tab, with icons (`icons` sheet), stats and descriptions. The EQUIPMENT screen previews the load of whatever you highlight before you equip it.
...
- Boss, enemy and player sounds are layered recipes (`src/audio/BossSfx.ts`): any effect id starting `b_` (bosses), `e_` (regular enemies) or `p_` (the player). They're built from whooshes, deep booms, rumble and debris, great bells, metal clangs, chains, wet splashes and bubbles, sizzling wax, fire, bone rattles, swelling spells, and growling or singing voices through vowel formants, all in a hall reverb and panned toward the boss. A strike's `windupSfx` plays as it starts winding up, the tell you hear: the Tollwarden's grunt and chain, the Matron's wet breath or whispered hush, Mother Tallow's hum, a bone rattle, the Chandler's chant. Its `sfx` plays on the swing or impact, and a lob's `sfx` where it lands. Bosses have their own footfalls too (`steps` on an enemy: a sound on each contact frame of its walk cycle, quieter with distance): the Tollwarden's iron boots and chains, the Matron's wet dragging, Mother Tallow's floor-shaking weight, bone on stone, the Chandler's robes and the tap of his snuffer, the Last Candle's burning tread. Regular enemies have the same (`e_` sounds) plus a `voice`: an `alert` cry when they spot you (the Wickling's squeak, the hound's bark, the Brute's roar, the Lantern's ringing alarm), a `hurt` sound on each hit, a `die` sound, and a `rise` for ambushers coming up out of the water. Every enemy sound id is checked when the data loads.
- The player's weapons each sound like themselves (weapon `sounds` in `data/weapons`). There's a `draw` as the weapon comes to hand (swap or equip), the strikes' own swings, a `hit` when they land (a blade biting, a knife stabbing, the axe's crunch, fists and gun-butts thudding), a `shotHit` for bullets and bolts, a `charge` breath as a heavy is drawn back, and the `reload` done in steps: the revolver's cylinder out, brass tinkling, six rounds in, a click shut; the flintlock's powder, three strokes of the ramrod, the cock; the crossbow's windlass ratchet, the bolt seated, the latch. The flintlock fires with a snap, a flash of the pan, then the boom. Shields have their own `blockSfx` and `parrySfx`. Rolls sound like your load (a light tumble, a roll, a heavy thud with mail, a clumsy fall). Heavy body armour jingles with each step. Hurt, dying and drinking have their own sounds, and so does putting gear on.
...
- Boss music (`src/audio/Music.ts`, `data/audio/music.json`) is a synthesised orchestra in a stone hall. Every section is several detuned voices spread across the stereo field: strings, spiccato ostinatos, an "aah" choir through vowel formants, French horns, brass, a pipe organ, celesta and harp over low strings. The percussion is taiko, pitched timpani, snare and cymbal swells. Everything plays into a generated cathedral reverb. Each boss phase has its own theme, fast in the Dark Souls manner (138-188 bpm): a chord progression with layered step-pattern parts and melodies doubled across horns, strings and choir. A theme's `form` gives it a song structure. A verse runs on the ostinato and light drums. A build adds choir chants, a riser and a snare roll, then everything cuts out for a beat (`gap`). Then the drop: taiko, timpani, syncopated orchestral hits, the chanting choir and the melody, landing on a crash. A quiet breakdown follows, then another build and drop. Chords are voiced wide ("1, 5, 1 5"). Layers come in by intensity: 0 during the boss's entrance, 1 in the fight, 2 below half health. Between phases only the `hold` layers continue. When the last phase falls, the whole orchestra lands on a major chord with a timpani roll and a cymbal; if you die, the music fades. The ambience ducks under it. Themes:
  - Tollwarden, "Toll" (156 bpm): a heroic D-minor charge (i–VI–III–VII) with horn fanfares, a tolling bell and chanting choir. Brass and high strings double the melody below half health.
...
- Exploration music (`areas` and `explore` in `data/audio/music.json`, played by `ExploreMusic`) is kept in the Dark Souls manner: very quiet and slow (50-72 bpm), with no drums. It plays only a pad, a soft choir and one melody. Each 8-bar phrase is followed by 12 bars of silence, so mostly you hear the place itself. It crossfades when the area changes, fades out for a boss fight and returns 12 s after, and ducks under Wren's harp in the hub. Themes:
  - Penance Road: a lonely pilgrims' air in D minor, a far-off horn over strings.
  - Wick's Rest: warm and safe, F major, celesta over strings and a soft choir.
  - The Waxmire: the Matron's lullaby remembered on a music box over a low choir and a little harp.
  - The Tallow Works: phrygian, low strings and a horn in the smoke.
