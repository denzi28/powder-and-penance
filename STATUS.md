...
- **Pixel-art pass started** (`tools/gen-art.ts`; regenerate single sheets with `npm run gen:art -- --only=tiles,player_body`):
  ...
  - Expansion Step 4, Bloomhollow: 10 orchard rooms through the collapsed gate on Penance Road (`tools/level-gen/bloom.mjs`, area `bloom`, tileset `tiles_orchard`, `decor_bloom`). Hives release swarms that chase whoever struck them and sting everyone in their cloud (`src/game/Swarms.ts`, `data/swarms.json`, prop `hive`); smoke calms swarms and blinds (strike `smoke`, `player.blind` / `enemy.setBlind`, the UIScene darkness); honey floors and pools slow; tallow seals melt near a beeswax light (prop `melts`, mod `light`). Beekeeper Husks, Drone Swarms, Orchard Guards (visible ambushers), Honey Slimes; the Scarecrow Warden (miniboss); the Hive Queen and her Swarm (a `turn` at the Great Skep). Hild, Maudlin's sister, at the Press House: the Smoker, her apiary shop (`seller: "hild"` in `data/shop.json`, `{ "open": "apiary" }`), and the letter quest to Maudlin. Veil, Coat, Beeswax Candle, Ring of the Queen (mods `stings`, `critHeal`, `light`), six notes, the arrival cutscene, Mother Aldith's last words; orchard ambience (bees, songbirds), a waltz, two boss themes.
  - Act 1 route and balance: every biome is now on the way to the Chandler. The Nave door wants four Seals (Tallow, the Mire, Powder from the Gunner, the Hive from the Queen). Bloomhollow's collapsed gate is blocked by rubble that only a powder blast clears (prop `blastOnly` with a `minRadius`: a Keg Charge does it, a firebomb doesn't); the Gunner hands over two Keg Charges and Oskar sells more. Tomas and Oskar point the way. Areas carry a `tier`, a `tallowMult` and an `hpMult` (`data/areas.json`); minibosses can set their own `tallow`. Bosses retuned along the curve: Tollwarden 720 HP, Mother Tallow and the Matron 1,300 each, the Gunner and the Queen 1,650 each, the Chandler 2,000 with the hardest hit (64). The Powder Mule's death blast is 38. Ember Salt moved so +4 comes in tier 3 and +5 before the Chandler (the Works' and Mire's salts are now Tallow lumps; one is in Oskar's cache in the Vault; Oskar's salt waits for the Gunner). Levels cost less at the top (`quad` 8 → 6). `npm run balance` prints the expected player per tier against every enemy and boss; `tests/balance.test.ts` and `tests/route.test.ts` hold the curve and the route.
  - Art pass on the new biomes (the Vault's creatures were repainted earlier): Bloomhollow's Beekeeper Husk (a woven hat, a face behind black gauze, sleeves and gloves), Orchard Guard (a stitched sack head, a battered felt hat, lapels and patches, straw bursting from hems and cuffs), Honey Slime (its own glossy amber body with a comb scrap and drowned bees), the Scarecrow Warden (a candle-lit sack face, a crooked hat, crows, a glowing burnt hem, the keeper's lantern), the Hive Queen (coiled, bound straw; a comb bodice, hair, a face, gauze wings) and her Swarm (a gowned figure of bees lit gold on one side, a glowing comb heart). Orchard paths in soft patches with a ragged edge, meadow beds that spill flowers and tall grass, coil-shaded skeps, a matching scarecrow. The Powder Vault gets its own tileset, `tiles_vault` (its rooms were drawn in plain Abbey stone).
  - Sound fix (reported: boss fights' sound broke up about 10 s in): finished notes and effects stayed connected to the long-lived buses and piled up until the audio thread fell behind. Every note and effect now disconnects itself after its tail (`src/audio/release.ts`); music voices come from a cost-weighted budget that backs off if the audio clock falls behind (`src/audio/Music.ts`); at most 8 layered effects at once (`src/audio/BossSfx.ts`, which also lost its second convolver). Every boss fight now holds real time.
