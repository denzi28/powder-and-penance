...
// ---------------------------------------------------------------- small font (original 3x5, for item descriptions)
const GLYPHS_SMALL: Record<string, string> = {
  ' ': '...|...|...|...|...',
  '!': '.#.|.#.|.#.|...|.#.',
  '"': '#.#|#.#|...|...|...',
  '#': '#.#|###|#.#|###|#.#',
...
function genSmallFont() {
  const img = new Img(64, 36);
  for (let code = 32; code <= 126; code++) {
    const g = GLYPHS_SMALL[String.fromCharCode(code)] ?? GLYPHS_SMALL['?'];
    const i = code - 32;
    const ox = (i % 16) * 4;
    const oy = Math.floor(i / 16) * 6;
    g.split('|').forEach((row, y) => [...row].forEach((p, x) => p === '#' && img.set(ox + x, oy + y, P.white)));
  }
  write(path.join(FONTS, 'pixel3x5.png'), img.toBuffer());
}
